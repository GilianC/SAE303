...
RentalData.fetchAllMovie = async function() {
    let data = await getRequest('rental?param=rentalmoviestat');
    console.log(data);
    return data;
};
RentalData.fetchMoviePerStat = async function(id) {
    let data = await getRequest("rental?id="+id);
    return data;
};
export {RentalData};